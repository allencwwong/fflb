  ...
  $.ajax({
    ...
    success: function (whss) {
      ...
      console.log(whss);
        ...
        // whss.forEach(function(whs,idx){

        //   cardBodyHtml += `<div class="col-12 col-lg-3 col-md-6 mb-3"><div class="card card-gold card-long my-3"><div class=card-body><div class=row><div class=col-md-12><div class=card-header>Week${whs.week} %></div><div class=py-3><img class=team-icon src="${whs.team.profileImg}"></div></div><div class="col-md-12 text-center"><div class=row><h1 class="col-12 team-heading">${whs.team.teamName}</h1><div class="col-12 pb-2 px-3 pt-1"><div class=team-info>${whs.team.leagueName}</div></div><div class="col-12 pb-2 px-3"><div class=team-info>${whs.team.manager}</div></div></div><div class="mb-3 mt-2"><div class=team-ts>High Score</div><h2 class=team-heading2>${whs.whs.score}</h2></div><div class=row><div class="col-5 offset-1"><div>W - L - T</div><div class=ft-ysanbold>${whs.team.standing.outcome_totals.wins}-${whs.team.standing.outcome_totals.losses}-${whs.team.standing.outcome_totals.ties}</div></div><div class=col-5><div>W/L Ratio</div><div class=ft-ysanbold>${(whs.team.standing.outcome_totals.percentage * 100).toFixed(2)}%</div></div></div></div></div></div><div class="card-footer card-league-footer"data-target=#myModal data-toggle=modal>view more</div></div></div>`;
        // })
